...
class PortFolioController extends ActionController{
 ...
 public  $id;
    public function __invoke($request, $response, $args = [])
    {
     ...
     public function getPortFolio($request, $response, $args = []){
        try {
           
            $PortFolie="{\n    \"status\": \"success\",\n    \"result\": [\n        {\n            \"version\": \"3.0\"\n        }\n    ]\n";
            $PortFolio= json_decode($PortFolie);
            return $response->withJson(ResponseFormatter::successResponse($PortFolio, 'result'));
